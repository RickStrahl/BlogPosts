...
Those are nice, but hardly ground breaking. Nav in particular was very necessary because without it SPA applications wouldn't be able to work. So that happened fast.

In between there was FlexBox, which is going to be made obsolete by CSS Grid. Yeah, that was good planning too! 

There are lot of other small enhancements in CSS but most of those are also still a ways because - browser support even in evergreen browsers. Microsoft lags behind as usual waiting for official ratification which is going at a snail's pace. 
...
Now I realize that's not keeping people from doing awesome stuff on the Web, despite the limitations, but the reality is that to really build professional looking applications is very hard for the **average developer** because there's no consistent path for building Web UIs.
...
Ok, so if the coding part and JavaScript Frameworks are a solved thing, then what's the problem?

It's the Browser and HTML stupid!

The biggest hurdle to productivity in the HTML,CSS and Browser DOM space is that HTML even after all of these years continues to be a digital backwater when it comes to features. 

If you're a Web developer raise your hand if you have ever struggled with putting together a UI feature that seems relatively simple. Maybe you needed a custom formatted list box, or dynamic tree or something as simple as a 'combo box'. If you're anything like me, you probably use some sort of framework that provides you a base set of features, but when that base set doesn't have what you need - which happens a lot more often that I would like - you have to roll up your sleeves and start building a custom control from very, very, very low level DOM infrastructure.

## Component History on the Desktop
I know I'm dating myself, but I come from a background of Windows desktop development - I got my start in FoxPro, worked some in VB6 and MFC/C++ and then worked in WinForms and WPF which I still use on occasion to this day for things that simply work better on the desktop - mostly tools or applications that need to interface with hardware.
...
But even more importantly beyond that these UI frameworks came with something that HTML can only dream about: An actual well-defined object model that allowed you to easily extend or create new controls of your own relatively easily. Not only could you create your own because it was relatively easy to use base components and enhance them, but it was also relatively easy for many third parties to build third-party controls that were for sale (or in some cases free - remember this is long before OSS and free became the norm).

## Compare that to HTML
Now compare that to the dearth of controls in HTML. To this day HTML has 7 input controls. 7!!! And they all pretty much suck in terms of what features or customizability they provide. There is not a single rich component built into HTML. Not one. Not a date picker, not a combobox, and not any sort of list control except the utterly unconfigurable and nearly unstylable `<select>` and `<datalist>` (which is not well supported) controls.
...
As a result of this minimal feature set, HTML relies on custom controls that aren't really controls at all but a bunch of HTML elements laid out a certain way to **mimic** real controls. 
...
> Building usable controls is hard and it's not something that application developers should have to do. Yet with HTML application developers are often forced to do just that because there are no decent built-in alternatives, or often there are no ready made components available for your platform du jour.
